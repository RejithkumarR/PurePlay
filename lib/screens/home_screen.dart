...
class _HomeScreenState extends State<HomeScreen> with SingleTickerProviderStateMixin {
    ...
    if (mounted) {
      setState(() {
        ...
        if (_openFolderPath != null &&
            !_foldersFor(_currentType).any((f) => f.path == _openFolderPath)) {
  ...
  List<MediaFile> _filtered(MediaType type) {
    ...
    return _all.where((m) {
      ...
      return q.isEmpty || m.title.toLowerCase().contains(q) || m.folderName.toLowerCase().contains(q);
    }).toList();
  }
  ...
  List<_FolderEntry> _foldersFor(MediaType type) {
    ...
    for (final media in _filtered(type)) {
      final key = media.relativePath.isNotEmpty
          ? media.relativePath
          : media.folderName;
      grouped.putIfAbsent(key, () => []).add(media);
    }
    final folders = grouped.entries
        .map((entry) => _FolderEntry(
              path: entry.key,
              name: _displayFolderName(entry.key),
              files: entry.value,
            ))
        .toList();
    folders.sort((a, b) => _compareText(a.name, b.name));
    if (!_ascending) return folders.reversed.toList();
    return folders;
  }
  ...
  String _displayFolderName(String path) {
    ...
    return parts.last.isEmpty ? 'Internal storage' : parts.last;
  }
  ...
  List<MediaFile> _sortedFiles(List<MediaFile> files) {
    final result = [...files];
    result.sort((a, b) {
      final comparison = _sortMode == _SortMode.name
  ...
  void _openFolder(_FolderEntry folder) => setState(() => _openFolderPath = folder.path);

  void _closeFolder() => setState(() => _openFolderPath = null);
  ...
  Future<void> _copyOrMove(MediaFile media, {required bool move}) async {
    final folders = _foldersFor(media.type)
        .where((folder) => folder.path != media.relativePath)
        .toList();
    if (folders.isEmpty) {
      ...
      child: Scaffold(
        appBar: AppBar(
          leading: _openFolderPath != null
              ? IconButton(icon: const Icon(Icons.arrow_back), onPressed: _closeFolder)
              : null,
          title: Row(children: [
          ...
          bottom: TabBar(
            ...
            tabs: const [Tab(text: 'Videos', icon: Icon(Icons.video_library_rounded)), Tab(text: 'Audio', icon: Icon(Icons.library_music_rounded))],
          ),
        ),
        body: _loading
  ...
  Widget _content(MediaType type) {
    ...
    return RefreshIndicator(
      onRefresh: _load,
      child: _viewMode == _ViewMode.grid ? GridView.builder(
        ...
        itemBuilder: (_, i) => _folderCard(folders[i]),
      ) : ListView.separated(
        padding: const EdgeInsets.symmetric(vertical: 8),
        itemCount: folders.length,
        separatorBuilder: (_, __) => const Divider(height: 1, indent: 72),
        itemBuilder: (_, i) => _folderListTile(folders[i]),
      ),
    );
  }
  ...
  Widget _folderCard(_FolderEntry folder) => Card(
    ...
    child: InkWell(onTap: () => _openFolder(folder), child: Padding(padding: const EdgeInsets.all(16), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
  ...
  Widget _fileView(List<MediaFile> items) {
    ...
    return RefreshIndicator(
      ...
      child: _viewMode == _ViewMode.grid ? GridView.builder(
        ...
        itemCount: items.length,
        separatorBuilder: (_, __) => const Divider(height: 1, indent: 72),
        itemBuilder: (_, i) => _fileListTile(items, i),
      ),
    );
  }
  ...
  Widget _fileCard(List<MediaFile> items, int index) {
    final media = items[index];
    return Card(clipBehavior: Clip.antiAlias, child: InkWell(onTap: () => _openMedia(items, index), onLongPress: () => _fileMenu(media), child: Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Expanded(child: Container(width: double.infinity, decoration: BoxDecoration(color: AppColors.surface2, borderRadius: BorderRadius.circular(12)), child: Icon(media.type == MediaType.video ? Icons.movie_rounded : Icons.music_note_rounded, size: 52, color: AppColors.accent))),
      const SizedBox(height: 8),
      Row(children: [Expanded(child: Text(media.title, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w600))), IconButton(iconSize: 20, visualDensity: VisualDensity.compact, onPressed: () => _fileMenu(media), icon: const Icon(Icons.more_vert_rounded))]),
