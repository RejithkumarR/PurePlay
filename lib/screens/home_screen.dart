...
class _HomeScreenState extends State<HomeScreen> with SingleTickerProviderStateMixin {
  ...
  List<MediaFile> _filtered(MediaType type) {
    ...
    return _all.where((m) {
      ...
      return q.isEmpty || m.title.toLowerCase().contains(q) || m.folderName.toLowerCase().contains(q) || m.relativePath.toLowerCase().contains(q);
    }).toList();
  }
  ...
  List<_FolderEntry> _foldersFor(MediaType type) {
    ...
    final folders = grouped.entries.map((entry) => _FolderEntry(path: entry.key, name: _displayFolderName(entry.key), files: entry.value)).toList();
  ...
  Future<void> _openMedia(List<MediaFile> items, int index) async {
    ...
    if (media.type == MediaType.video) {
      await Navigator.push(context, MaterialPageRoute(builder: (_) => VideoPlayerScreen(media: media, playlist: items, initialIndex: index)));
    } else {
    ...
    final destination = await showModalBottomSheet<_FolderEntry>(
      ...
      builder: (context) => SafeArea(
        child: ListView(shrinkWrap: true, children: [
          Padding(padding: const EdgeInsets.fromLTRB(20, 4, 20, 12), child: Text(move ? 'Move to folder' : 'Copy to folder', style: Theme.of(context).textTheme.titleLarge)),
          ...folders.map((folder) => ListTile(leading: Icon(Icons.folder_rounded, color: AppColors.accent), title: Text(folder.name), subtitle: Text(folder.path), onTap: () => Navigator.pop(context, folder))),
        ]),
      ),
    );
  ...
  Widget build(BuildContext context) {
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          leading: _openFolderPath != null ? IconButton(icon: const Icon(Icons.arrow_back), onPressed: () => setState(() => _openFolderPath = null)) : null,
          title: Row(children: [
          ...
          bottom: TabBar(controller: _tabs, onTap: (_) => setState(() => _openFolderPath = null), tabs: const [Tab(text: 'Videos', icon: Icon(Icons.video_library_rounded)), Tab(text: 'Audio', icon: Icon(Icons.library_music_rounded))]),
        ),
        body: _loading
            ? const Center(child: CircularProgressIndicator())
            : Column(children: [
                Padding(padding: const EdgeInsets.fromLTRB(16, 14, 16, 6), child: TextField(decoration: const InputDecoration(prefixIcon: Icon(Icons.search), hintText: 'Search local media...', border: OutlineInputBorder()), onChanged: (v) => setState(() => _query = v))),
  ...
  Future<void> _showSortSheet() async {
    final choice = await showModalBottomSheet<Map<String, dynamic>>(
      ...
      builder: (context) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        RadioListTile<_SortMode>(value: _SortMode.name, groupValue: _sortMode, title: const Text('Name'), onChanged: (v) => Navigator.pop(context, {'mode': v, 'ascending': _ascending})),
        RadioListTile<_SortMode>(value: _SortMode.date, groupValue: _sortMode, title: const Text('Date modified'), onChanged: (v) => Navigator.pop(context, {'mode': v, 'ascending': _ascending})),
        SwitchListTile(title: const Text('Ascending'), value: _ascending, onChanged: (v) => Navigator.pop(context, {'mode': _sortMode, 'ascending': v})),
  ...
  Widget _content(MediaType type) {
    if (_openFolderPath != null) {
      final folder = _foldersFor(type).where((f) => f.path == _openFolderPath).firstOrNull;
    ...
    if (folders.isEmpty) return Center(child: Text(_showFavorites ? 'No favorite ${type.name} files' : 'No ${type.name} files found'));
    return RefreshIndicator(onRefresh: _load, child: _viewMode == _ViewMode.grid
        ? GridView.builder(padding: const EdgeInsets.all(14), gridDelegate: const SliverGridDelegateWithMaxCrossAxisExtent(maxCrossAxisExtent: 190, childAspectRatio: 1.12, crossAxisSpacing: 10, mainAxisSpacing: 10), itemCount: folders.length, itemBuilder: (_, i) => _folderCard(folders[i]))
        : ListView.separated(padding: const EdgeInsets.symmetric(vertical: 8), itemCount: folders.length, separatorBuilder: (_, __) => const Divider(height: 1, indent: 72), itemBuilder: (_, i) => _folderListTile(folders[i])));
  }

  Widget _folderCard(_FolderEntry folder) => Card(clipBehavior: Clip.antiAlias, child: InkWell(onTap: () => setState(() => _openFolderPath = folder.path), child: Padding(padding: const EdgeInsets.all(16), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.folder_rounded, size: 54, color: AppColors.accent), const SizedBox(height: 10), Text(folder.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w700)), const SizedBox(height: 4), Text('${folder.files.length} ${folder.files.length == 1 ? 'file' : 'files'}', style: TextStyle(color: AppColors.muted))]))));

  Widget _folderListTile(_FolderEntry folder) => ListTile(contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 5), leading: CircleAvatar(backgroundColor: AppColors.surface2, child: Icon(Icons.folder_rounded, color: AppColors.accent)), title: Text(folder.name, maxLines: 1, overflow: TextOverflow.ellipsis), subtitle: Text('${folder.files.length} files • ${folder.path}', maxLines: 1, overflow: TextOverflow.ellipsis), trailing: const Icon(Icons.chevron_right_rounded), onTap: () => setState(() => _openFolderPath = folder.path));
  ...
  Widget _fileCard(List<MediaFile> items, int index) {
    final media = items[index];
    return Card(clipBehavior: Clip.antiAlias, child: InkWell(onTap: () => _openMedia(items, index), onLongPress: () => _fileMenu(media), child: Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Expanded(child: Container(width: double.infinity, decoration: BoxDecoration(color: AppColors.surface2, borderRadius: BorderRadius.circular(12)), child: Icon(media.type == MediaType.video ? Icons.movie_rounded : Icons.music_note_rounded, size: 52, color: AppColors.accent))), const SizedBox(height: 8), Row(children: [Expanded(child: Text(media.title, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w600))), IconButton(iconSize: 20, visualDensity: VisualDensity.compact, onPressed: () => _fileMenu(media), icon: const Icon(Icons.more_vert_rounded))]), Text('${media.formattedSize} • ${DateFormat("dd MMM yyyy").format(media.modifiedDate)}', style: TextStyle(color: AppColors.muted, fontSize: 11))]))));
  }
